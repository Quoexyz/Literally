...
# 模式选择
mode_var = tk.StringVar(value=MODE_LLM)
mode_menu = ctk.CTkOptionMenu(
    model_frame, values=[MODE_LLM, MODE_TRANSLIT],
    variable=mode_var, width=180, height=32,
    corner_radius=8, font=ctk.CTkFont(size=12),
    fg_color="#475569", button_color="#334155", button_hover_color="#1E293B",
)
...
def translate():
    ...
    if mode_var.get() == MODE_TRANSLIT:
        translate_translit(text)
        return
...
def translate_translit(text):
    """发音模仿模式：json 词表 -> ONNX 音译模型 -> 规则引擎兜底（引擎内部链）。"""
    def _run():
        status_var.set("音译中...")
        try:
            result, sources = translit_engine.transliterate(text)
            stats = {}
            for s in sources:
        ...
        def _show():
            ...
            output_box.configure(state="disabled")
            status_var.set(f"Ready   [{detail}]" if result else f"Error: {detail}")
